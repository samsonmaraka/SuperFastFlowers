...
export function AdminProductsClient({ initial }: { initial: Product[] }) {
  ...
  function calculateOrderDeliveryFee(order: OrderRequest) {
    ...
    for (const item of order.items || []) {
      ...
      const itemCost = distanceKm * 1000;
      if (itemCost > maxCost) maxCost = itemCost;
    }
  ...
  return <div className='space-y-4'>
    ...
    {tab==='orders' && <div className='space-y-4'><div className='flex gap-2'><button className='rounded border px-2 py-1' onClick={()=>fetchOrders('active')}>Active orders</button><button className='rounded border px-2 py-1' onClick={()=>fetchOrders('closed')}>Closed orders</button><button className='rounded border px-2 py-1' onClick={()=>fetchOrders()}>All</button></div>{orders.map(o=>{ const deliveryFee = calculateOrderDeliveryFee(o); return <div key={o.id} className='rounded border bg-white p-3 text-sm'><div className='mb-1 flex justify-between'><span><b>Order ID:</b> {o.id} · <b>Order datetime:</b> {formatOrderDateTime(o.createdAt)}</span><span>{o.status}</span></div><p><b>Recipient name:</b> {o.recipientName} · <b>Recipient contact:</b> {o.recipientPhone} · {o.email}</p><p><b>Recipient location:</b> {o.region}/{o.cityId}{o.deliveryPinUrl ? ` · ${o.deliveryPinUrl}` : ''}</p><p><b>Order items:</b> {(o.items||[]).map(i=>`${i.name||i.productId} x${i.quantity}`).join(', ')||'No items'}</p><p><b>Estimated delivery fee:</b> {deliveryFee === null ? 'N/A (missing recipient coordinates)' : `UGX ${deliveryFee.toLocaleString()} @ UGX 1,000/km`}</p><p><b>Order total:</b> {o.totalAmount ?? 'N/A'}</p><select value={statusOptions.includes(o.status)?o.status:'new'} onChange={e=>updateStatus(o.id,e.target.value as OrderStatus)} className='rounded border p-1'>{statusOptions.map(s=><option key={s} value={s}>{s}</option>)}</select>{activeStatuses.includes(o.status)?<span className='ml-2 text-orange-600'>Active</span>:<span className='ml-2 text-green-700'>Closed</span>}</div>;})}</div>}
