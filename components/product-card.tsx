...
export function ProductCard({ product }: { product: Product }) {
  return (
    <article className="relative overflow-hidden rounded-xl border border-blush bg-white shadow-sm transition-shadow hover:shadow-md">
      ...
      <Link href={`/shop/${product.slug}`} className="absolute inset-0 z-10 focus:outline-none focus:ring-2 focus:ring-pink-700 focus:ring-offset-2" aria-label={`View ${product.name}`} />
    </article>
  );
}
